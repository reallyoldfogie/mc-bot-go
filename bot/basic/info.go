...
func (p *player) handleLoginPacket(packet pk.Packet) error {
	...
	// NOTE: Brand and ClientInformation packets are now sent by mc-agent
	// using version-specific handlers. This ensures compatibility across
	// different Minecraft versions (e.g., 1.21.1 vs 1.21.4+).
	// See mc-agent/agent/hotbar.go initClientInformationHandler()
	//
	// err = p.client.Conn().WritePacket(pk.Marshal( // PluginMessage packet
	// 	p.packetMgr.GetServerboundPacketID("ServerboundCustomPayload"),
	...
	// 	p.packetMgr.GetServerboundPacketID("ServerboundClientInformation"),
	...
	// 	pk.VarInt(0), // ParticleStatus - only in 1.21.4+
	// ))
	// if err != nil {
	// 	return Error{err}
	// }
