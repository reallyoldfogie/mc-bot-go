...
func (e EventsListener) attach(p Player, packetMgr models.PacketMgr) {
	if e.GameStart != nil {
		attachJoinGameHandler(p.Client(), packetMgr, e.GameStart)
	}
	if e.Disconnect != nil {
		attachDisconnect(p.Client(), packetMgr, e.Disconnect)
	}
	if e.HealthChange != nil || e.Death != nil {
		attachUpdateHealth(p.Client(), packetMgr, e.HealthChange, e.Death)
	}
	if e.Teleported != nil {
		attachPlayerPosition(p.Client(), packetMgr, e.Teleported)
	}
}

func attachJoinGameHandler(c bot.Client, packetMgr models.PacketMgr, handler func() error) {
	c.Events().AddListener(bot.PacketHandler{
		Priority: 64, ID: packetMgr.GetClientboundPacketID("ClientboundLogin"),
...
func attachDisconnect(c bot.Client, packetMgr models.PacketMgr, handler func(reason chat.Message) error) {
	c.Events().AddListener(bot.PacketHandler{
		Priority: 64, ID: packetMgr.GetClientboundPacketID("ClientboundDisconnect"),
...
func attachUpdateHealth(c bot.Client, packetMgr models.PacketMgr, healthChangeHandler func(health float32, food int32, saturation float32) error, deathHandler func() error) {
	c.Events().AddListener(bot.PacketHandler{
		Priority: 64, ID: packetMgr.GetClientboundPacketID("ClientboundSetHealth"),
		F: func(p pk.Packet) error {
			var health pk.Float
			var food pk.VarInt
...
func attachPlayerPosition(c bot.Client, packetMgr models.PacketMgr, handler func(x, y, z float64, yaw, pitch float32, flag byte, teleportID int32) error) {
	c.Events().AddListener(bot.PacketHandler{
		Priority: 64, ID: packetMgr.GetClientboundPacketID("ClientboundPlayerPosition"),
		F: func(p pk.Packet) error {
			var (
				X, Y, Z    pk.Double
				Yaw, Pitch pk.Float
